namespace GuardClausesUnitTest;
...
public class ArgumentExceptionTests
{
    ...
    [Test]
    public void AndTestWithInvalidValue()
    {
        _ = Invoking(() => Guard.Against.Null("Jo").And(x => x.Length >= 3, "name"))
            .Should()
            .Throw<ArgumentException>()
            .WithMessage("Input name did not satisfy the conditions (x => x.Length >= 3). (Parameter 'name')")
            .WithParameterName("name");
    }

    [Test]
    public void AndTestWithInvalidNullValue()
    {
        string value = null!;

        _ = Invoking(() => Guard.Against.Null(value).And(x => x?.Length >= 3, "name"))
            .Should()
    ...
    [Test]
    public void AndTestWithValidValue()
    {
        _ = Invoking(() => Guard.Against.Null("Joe").And(x => x.Length >= 3, "name"))
            .Should()
            .NotThrow();
    }
}